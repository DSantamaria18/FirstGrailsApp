...
<div class="fieldcontain ${hasErrors(bean: taskInstance, field: 'assignedTo', 'error')} ">
	...
	<g:select id="assignedTo" name="assignedTo.id" from="${com.talkio.User.list()}" optionKey="id" value="${taskInstance?.assignedTo?.id}" class="many-to-one" noSelection="['null': '']"/>
...
<div class="fieldcontain ${hasErrors(bean: taskInstance, field: 'event', 'error')} required">
	...
	<g:select id="event" name="event.id" from="${com.talkio.Event.list()}" optionKey="id" required="" value="${taskInstance?.event?.id}" class="many-to-one"/>
